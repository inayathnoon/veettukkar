...
export default function RateHomeownerScreen() {
  const { t } = useTranslation();
  const router = useRouter();
  const { jobId, homeownerUid } = useLocalSearchParams<{ jobId: string; homeownerUid: string }>();
  ...
  const handleSubmit = async () => {
    ...
    if (!jobId || !homeownerUid) {
      Alert.alert(t('common.error'), 'Missing job or homeowner information');
      return;
    }

    const result = await submitRating(jobId, homeownerUid, 'worker_to_homeowner', stars, comment);
    if (result.success) {
      router.back();
    } else {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.prompt}>{t('rating.homeowner_prompt')}</Text>
